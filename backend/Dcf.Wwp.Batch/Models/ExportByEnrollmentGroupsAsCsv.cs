...
namespace Dcf.Wwp.Batch.Models
{
    ...
    /// </summary>
    public class ExportByEnrollmentGroupsAsCsv : IExportable
    {
        #region Properties
        ...
        public ExportByEnrollmentGroupsAsCsv(IBatchJobOptions options, ILog log)
        {
            ...
            using (var ms = new MemoryStream())
            {
                using (var fileWriter = new DelimitedFileWriter(ms, ",", Encoding.ASCII))
                {
                    ...
                    if (iNumRows > 0)
                    {
                        ...
                        var iFilterRows = dataView.Count;

                        _log.Info("");
                        _log.Info($"-- CoEnrolled ({iFilterRows}) rows -- REQUIRES INDIVIDUAL ANALYSIS --");

                        for (var rowIdx = 0; rowIdx < iFilterRows; rowIdx++)
                        {
                        ...
                        // second group 
                        dataView.RowFilter = "CoEnrolled = false";
                        iFilterRows        = dataView.Count;
                        _log.Info("");
                        _log.Info($"-- Non-CoEnrolled ({iFilterRows}) rows -- DISENROLLMENT COMPLETED --");
