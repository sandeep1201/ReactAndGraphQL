...
namespace DCF.Core
{
    public static class ObjectExtensions
    {
        ...
        {
            return JsonConvert.SerializeObject(val, Formatting.Indented,new JsonSerializerSettings() {MaxDepth = 2, ReferenceLoopHandling = ReferenceLoopHandling.Ignore});
        }
        ...
        public static DataTable ExecSql(this SqlConnection cn, CommandType commandType, string sql, List<SqlParameter> sqlParms = null)
        {
            ...
            using (var cmd = cn.CreateCommand())
            {
                ...
                {
                    if (sqlParms != null && sqlParms.Any())
                        sqlParms.ForEach(sp => cmd.Parameters.Add(sp));
                }
                else
                {
                    if (sqlParms != null && sqlParms.Any())
