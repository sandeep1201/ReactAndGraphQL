...
namespace Dcf.Wwp.Api.Controllers.InformalAssessment
{
    ...
    public class HousingController : BaseController
    {
        ...
        public HousingController(IAuthUser authUser, IRepository repository) : base(repository)
        {
            _authUser = authUser;
        }
        ...
        public IActionResult GetSection(string pin)
        {
            try
            {
                var vm = new HousingSectionViewModel(Repo, _authUser);
        ...
        [HttpPost("{pin}")]
        [EnableCors("AllowAll")]
        public IActionResult PostSection(string pin, [FromBody] HousingSectionContract model)
        {
