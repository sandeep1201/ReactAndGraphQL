...
{
    [Route("api/ia/non-custodial-parents")]
    [EnableCors("AllowAll")]
    [ServiceFilter(typeof(ValidAuthUserMustExistAttribute))]
    public class NonCustodialParentsController : BaseController
    {
        private readonly IAuthUser _authUser;

        public NonCustodialParentsController(IAuthUser authUser, IRepository repository) : base(repository)
        {
            _authUser = authUser;
        }
        ...
        public IActionResult GetSection(string pin)
        {
            try
            {
                var vm = new NonCustodialParentsSectionViewModel(Repo, _authUser);
        ...
        [HttpPost("{pin}")]
        [EnableCors("AllowAll")]
        public IActionResult PostSection(string pin, [FromBody] NonCustodialParentAssessmentContract model)
        {
