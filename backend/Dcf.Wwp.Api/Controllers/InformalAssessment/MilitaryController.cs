...
using Dcf.Wwp.Api.Library.Error;
...
namespace Dcf.Wwp.Api.Controllers.InformalAssessment
{
    ...
    public class MilitaryController : BaseController
    {
        ...
        public MilitaryController(IAuthUser authUser, IRepository repository) : base(repository)
        {
            _authUser = authUser;
        }
        ...
        public IActionResult GetSection(string pin)
        {
            try
            {
                var vm = new MilitarySectionViewModel(Repo, _authUser);
        ...
        [HttpPost("{pin}")]
        [EnableCors("AllowAll")]
        public IActionResult PostSection(string pin, [FromBody] MilitarySectionContract model)
        {
