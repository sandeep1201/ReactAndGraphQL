...
{
    [Route("api/ia/participant-barriers")]
    [ServiceFilter(typeof(ValidAuthUserMustExistAttribute))]
    public class ParticipantBarrierController : BaseController
    {
        private readonly IAuthUser _authUser;

        public ParticipantBarrierController(IAuthUser authUser, IRepository repository) : base(repository)
        {
            _authUser = authUser;
        }
        ...
        public IActionResult GetSection(string pin)
        {
            try
            {
                var vm = new ParticipantBarrierSectionViewModel(Repo, _authUser);
        ...
        [HttpPost("{pin}")]
        [EnableCors("AllowAll")]
        public IActionResult PostSection(string pin, [FromBody] ParticipantBarrierSectionContract model)
        {
