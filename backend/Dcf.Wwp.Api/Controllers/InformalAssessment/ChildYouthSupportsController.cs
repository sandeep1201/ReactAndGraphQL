...
{
    [Route("api/ia/child-youth-supports")]
    [EnableCors("AllowAll")]
    [ServiceFilter(typeof(ValidAuthUserMustExistAttribute))]
    public class ChildYouthSupportsController : BaseController
    {
        private readonly IAuthUser _authUser;

        public ChildYouthSupportsController(IAuthUser authUser, IRepository repository) : base(repository)
        {
            _authUser = authUser;
        }
        ...
        public IActionResult GetSection(string pin)
        {
            try
            {
                var vm = new ChildYouthSupportsSectionViewModel(Repo, _authUser);
        ...
        [HttpPost("{pin}")]
        [EnableCors("AllowAll")]
        public IActionResult PostSection(string pin, [FromBody] ChildYouthSupportsSectionContract model)
        {
