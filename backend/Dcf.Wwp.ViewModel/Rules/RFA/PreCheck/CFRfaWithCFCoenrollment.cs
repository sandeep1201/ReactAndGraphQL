...
{
    [Tag("RFA-PreCheck")]
    [Name("CFRfaWithCFCoenrollment")]
    public class CFRfaWithCFCoenrollment : Rule
    {
        public override void Define()
        {
            ...
            When()
                ...
                .Or(x => x
                    .And(xx => xx
                        .Match<IParticipant>(p => p.ParticipantEnrolledPrograms.Any(program => program.IsCF && (program.IsEnrolled || program.IsReferred)))
                        .Match<List<IRequestForAssistance>>(() => rfas, r => !r.Any(rs => rs.IsCF && (rs.IsEnrolled || rs.IsInProgress || rs.IsReferred))))
                    .And(xx => xx
                        .Match<List<IRequestForAssistance>>(() => rfas, r => r.Any(rs => rs.IsCF && (rs.IsEnrolled || rs.IsInProgress || rs.IsReferred)))));
            // .Let(() => ruleEnforcingRfa, () => rfas.FirstOrDefault(rs => rs.IsCF && (rs.IsEnrolled || rs.IsInProgress || rs.IsReferred)));
            ...
            Then()
                // "A new RFA for Children First cannot be created - Children First already exists".
                .Do(ctx => CreateMsg(messageCodeLevelResult));
        }

        private void CreateMsg(MessageCodeLevelContext messageCodeLevelResult)
        {
            const string code = Wwp.Model.Interface.Constants.RuleReason.CF;
