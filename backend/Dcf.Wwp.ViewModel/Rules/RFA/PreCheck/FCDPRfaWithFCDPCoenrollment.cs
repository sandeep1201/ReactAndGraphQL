...
{
    [Tag("RFA-PreCheck")]
    [Name("FCDPRfaWithFCDPCoenrollment")]
    public class FCDPRfaWithFCDPCoenrollment : Rule
    {
        public override void Define()
        {
            ...
            When()
                ...
                .Match<RequestForAssistanceContract>(() => contract, c => c.IsFCDP)
                .Or(x => x
                    .And(xx => xx
                        .Match<IParticipant>(p => p.ParticipantEnrolledPrograms.Any(program => program.IsFCDP && (program.IsEnrolled || program.IsReferred)))
                        .Match<List<IRequestForAssistance>>(() => rfas, r => !r.Any(rs => rs.IsFCDP && (rs.IsEnrolled || rs.IsInProgress || rs.IsReferred))))
                    .And(xx => xx
                        .Match<List<IRequestForAssistance>>(() => rfas, r => r.Any(rs => rs.IsFCDP && (rs.IsEnrolled || rs.IsInProgress || rs.IsReferred)))));
            // .Let(() => ruleEnforcingRfa, () => rfas.FirstOrDefault(rs => rs.IsFCDP && (rs.IsEnrolled || rs.IsInProgress || rs.IsReferred)));
