 ...
 * @param {function(Object,Object):boolean=} equalsFunction optional function used to
 * check equality between 2 elements.
 * @return {T} the  first occurrence of the specified element
 * within the specified array, or null if not found.
 */
export function find<T>(array: T[], item: T, equalsFunction?: util.IEqualsFunction<T>): T {
    const equals = equalsFunction || util.defaultEquals;
    const length = array.length;
    for (let i = 0; i < length; i++) {
        if (equals(array[i], item)) {
 ...
 * within the specified array.
 * @param {*} array the array in which to search the element.
 * @param {function(Object,index?,array?):boolean=} IFilterFunction optional function used to
 * test each value.
 * @param {Object} the default value to return if not found.
 * 
 * @return {T} the  first occurrence of the specified element
 * within the specified array, or null if not found.
 */
...
export function FirstOrDefault<T>(array: T[], predicate: util.IFilterFunction<T>, defaultValue: T = null ): T {
...
// TODO: Perf test this? 
export function unique<T, K>(array: T[], keySelector: util.IKeySelectorFunction<T, K>): T[] {
    let result: T[] = [];
    let map = new Map<K, T>();
