...
/**
 * show a selected date in monthly calenda.
 */
@Component({
  providers: [Datetime],
...
export class DatetimePickerComponent {
  ...
  @Input('allow-only-sundays') allowOnlySundays = false;
  ...
  public isDateDisabled(date: Date) {
    ...
    this.disabledDatesInTime = this.disabledDatesInTime || (this.disabledDates || []).map(d => d.getTime());
    ...
    if (this.allowOnlySundays && date.getDay() !== 0) {
      return true;
    }
